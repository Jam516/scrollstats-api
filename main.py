  ...
  if filter == 'all':
    ...
    WITH RankedProjects AS (
      SELECT 
        ...
        CASE 
            ...
            ELSE 'empty_call'
        END AS PROJECT,
        SUM((RECEIPT_L1_FEE + RECEIPT_EFFECTIVE_GAS_PRICE * RECEIPT_GAS_USED)/1e18) AS ETH_FEES,
        COUNT(DISTINCT u.FROM_ADDRESS) AS NUM_UNIQUE_WALLETS,
        ROW_NUMBER() OVER(PARTITION BY DATE_TRUNC('{time}', u.BLOCK_TIMESTAMP) ORDER BY SUM((RECEIPT_L1_FEE + RECEIPT_EFFECTIVE_GAS_PRICE * RECEIPT_GAS_USED)/1e18) DESC) AS RN
      FROM 
        SCROLL.RAW.TRANSACTIONS u
        LEFT JOIN SCROLL.RAW.CONTRACTS c ON u.TO_ADDRESS = c.ADDRESS
    ...
    aggregated_data AS (
        SELECT 
            ...
            SUM(CASE WHEN t.BLOCK_TIMESTAMP >= ts.one_{time}_ago THEN (t.RECEIPT_L1_FEE + t.RECEIPT_EFFECTIVE_GAS_PRICE * t.RECEIPT_GAS_USED)/1e18 END) AS gas_spend_current,
            SUM(CASE WHEN t.BLOCK_TIMESTAMP < ts.one_{time}_ago AND t.BLOCK_TIMESTAMP >= ts.two_{time}s_ago THEN (t.RECEIPT_L1_FEE + t.RECEIPT_EFFECTIVE_GAS_PRICE * t.RECEIPT_GAS_USED)/1e18 END) AS gas_spend_previous
        FROM 
...
def bd():
  ...
  aggregated_data AS (
      SELECT 
          ...
          SUM(CASE WHEN t.BLOCK_TIMESTAMP >= ts.one_month_ago THEN ((t.RECEIPT_L1_FEE + t.RECEIPT_EFFECTIVE_GAS_PRICE * t.RECEIPT_GAS_USED)/1e18) END) AS gas_spend_current,
          SUM(CASE WHEN t.BLOCK_TIMESTAMP < ts.one_month_ago AND t.BLOCK_TIMESTAMP >= ts.two_months_ago THEN ((t.RECEIPT_L1_FEE + t.RECEIPT_EFFECTIVE_GAS_PRICE * t.RECEIPT_GAS_USED)/1e18) END) AS gas_spend_previous
...
def economics():
  ...
  SELECT
      ...
      SUM(p.USD_PRICE * (RECEIPT_L1_FEE+RECEIPT_GAS_USED*RECEIPT_EFFECTIVE_GAS_PRICE)/1E18) AS GAS_REV_USD
  FROM SCROLL.RAW.TRANSACTIONS
  ...
  100* SUM((RECEIPT_GAS_USED*RECEIPT_EFFECTIVE_GAS_PRICE)/1E18)/SUM((RECEIPT_L1_FEE+RECEIPT_GAS_USED*RECEIPT_EFFECTIVE_GAS_PRICE)/1E18) AS l2_fee
  FROM SCROLL.RAW.TRANSACTIONS
  GROUP BY 1
  ''',
                         time=timeframe)
...
def econ_report():
  ...
  SUM((RECEIPT_L1_FEE+RECEIPT_GAS_USED*RECEIPT_EFFECTIVE_GAS_PRICE)/1E18) AS l2_gas_rev
  FROM SCROLL.RAW.TRANSACTIONS
